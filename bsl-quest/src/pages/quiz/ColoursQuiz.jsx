...
const ColoursQuiz = () => {
  ...
  const [currentQuestion, setCurrentQuestion] = useState(Math.floor(Math.random() * coloursData.length));
  ...
  useEffect(() => {
      ...
      setAnswered(false);
      console.log('answer is', coloursData[currentQuestion].colour)
  }, [currentQuestion]);
  ...
  const handleOptionClick = async (option) => {
    if (!answered) {
      ...
      const correctLetter = coloursData[currentQuestion].colour;
  ...
  const generateOptions = () => {
    let options = [coloursData[currentQuestion].colour];
    while (options.length < 4) {
      const randomIndex = Math.floor(Math.random() * coloursData.length);
      const colour = coloursData[randomIndex].colour;
  ...
  const pickNextQuestion = () => {
    ...
    let nextQuestionIndex = Math.floor(Math.random() * coloursData.length);
    if (incorrectAnswers.length > 0) {
        ...
        if (focusOnIncorrect) {
            ...
            const focusedIndex = coloursData.findIndex(item => item.colour === incorrectLetter);
            if (focusedIndex !== -1) {
  ...
  // Function to trigger confetti animation when the quiz is completed
  ...
  // Function to initialize user score document in Firestore so that we can save scores
  const initializeUserScore = async (userId) => {
    const userScoreRef = doc(db, "quizScores", userId);
    try {
        await setDoc(userScoreRef, {
          coloursQuizScores: [] // Initialize as an empty array if not present
  ...
  const saveQuizScore = async (score, totalXP) => {
    ...
    try {
      ...
      if (!docSnap.exists() || !docSnap.data().coloursQuizScores) {
  ...
  const removeLetterFromFocus = async (colour) => {
      const auth = getAuth();
      const user = auth.currentUser;
      if (!user) {
          console.error("No user is authenticated");
      ...
      try {
          const docSnap = await getDoc(userRef);
          if (docSnap.exists() && docSnap.data().incorrectAnswers) {
              const updatedIncorrectAnswers = docSnap.data().incorrectAnswers.filter(l => l !== colour);
  ...
  return (
    ...
    <div className="alphabetQuiz-container">            
      <h1>Spot the sign</h1>
      <video src={coloursData[currentQuestion].videoLink} controls playsInline muted autoPlay />
      <div>
        {options.map((option, index) => (
          <button 
            key={index}
            onClick={() => handleOptionClick(option)}
            className={`option-button ${answered ? (option === coloursData[currentQuestion].colour
